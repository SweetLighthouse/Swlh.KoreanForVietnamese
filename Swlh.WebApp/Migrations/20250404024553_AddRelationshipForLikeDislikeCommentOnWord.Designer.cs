// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Swlh.WebApp.Context;

#nullable disable

namespace Swlh.WebApp.Migrations
{
    [DbContext(typeof(MainDbContext))]
    [Migration("20250404024553_AddRelationshipForLikeDislikeCommentOnWord")]
    partial class AddRelationshipForLikeDislikeCommentOnWord
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.14")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Swlh.WebApp.Domain.Entities.Account", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("AccessedAcount")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(60)
                        .HasColumnType("nvarchar(60)");

                    b.Property<bool>("IsDisabled")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<string>("Password")
                        .IsRequired()
                        .HasMaxLength(60)
                        .HasColumnType("nvarchar(60)");

                    b.Property<int>("Role")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Username")
                        .IsRequired()
                        .HasMaxLength(60)
                        .HasColumnType("nvarchar(60)");

                    b.HasKey("Id");

                    b.ToTable("Account");
                });

            modelBuilder.Entity("Swlh.WebApp.Domain.Entities.CommentOnWord", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("AccountId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsDisabled")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<Guid>("WordId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("AccountId");

                    b.HasIndex("WordId");

                    b.ToTable("CommentOnWord");
                });

            modelBuilder.Entity("Swlh.WebApp.Domain.Entities.CommentOnWord_LikeDislike", b =>
                {
                    b.Property<Guid>("AccountId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CommentOnWordId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsLike")
                        .HasColumnType("bit");

                    b.HasKey("AccountId", "CommentOnWordId");

                    b.HasIndex("CommentOnWordId");

                    b.ToTable("CommentOnWord_LikeDislike");
                });

            modelBuilder.Entity("Swlh.WebApp.Domain.Entities.Example", b =>
                {
                    b.Property<string>("Korean")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Vietnamese")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Korean", "Vietnamese");

                    b.ToTable("Example");
                });

            modelBuilder.Entity("Swlh.WebApp.Domain.Entities.Word", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("MeaningKr")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("MeaningVn")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Pronunciation")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("WordKr")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("WordVn")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Word");
                });

            modelBuilder.Entity("Swlh.WebApp.Domain.Entities.CommentOnWord", b =>
                {
                    b.HasOne("Swlh.WebApp.Domain.Entities.Account", "Account")
                        .WithMany("CommentOnWords")
                        .HasForeignKey("AccountId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("Swlh.WebApp.Domain.Entities.Word", "Word")
                        .WithMany("CommentOnWords")
                        .HasForeignKey("WordId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Account");

                    b.Navigation("Word");
                });

            modelBuilder.Entity("Swlh.WebApp.Domain.Entities.CommentOnWord_LikeDislike", b =>
                {
                    b.HasOne("Swlh.WebApp.Domain.Entities.Account", "Account")
                        .WithMany("CommentOnWord_LikeDislike")
                        .HasForeignKey("AccountId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Swlh.WebApp.Domain.Entities.CommentOnWord", "CommentOnWord")
                        .WithMany("CommentOnWord_LikeDislikes")
                        .HasForeignKey("CommentOnWordId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Account");

                    b.Navigation("CommentOnWord");
                });

            modelBuilder.Entity("Swlh.WebApp.Domain.Entities.Account", b =>
                {
                    b.Navigation("CommentOnWord_LikeDislike");

                    b.Navigation("CommentOnWords");
                });

            modelBuilder.Entity("Swlh.WebApp.Domain.Entities.CommentOnWord", b =>
                {
                    b.Navigation("CommentOnWord_LikeDislikes");
                });

            modelBuilder.Entity("Swlh.WebApp.Domain.Entities.Word", b =>
                {
                    b.Navigation("CommentOnWords");
                });
#pragma warning restore 612, 618
        }
    }
}
